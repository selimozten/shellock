...
test("normal shellock startup creates settings without reading or copying Pi configuration", async () => {
  ...
  assert.equal(shellockSettings.shellockConfigVersion, 3);
...
test("shellock migrates Bedrock GPT models to the Mantle provider", async () => {
  const piAgentDir = await mkdtemp(join(tmpdir(), "pi-agent-"));
  const shellockAgentDir = await mkdtemp(join(tmpdir(), "shellock-agent-"));
  const cli = resolve("dist/distro/cli.js");
  ...
  assert.deepEqual(shellockSettings.enabledModels, [
    "amazon-bedrock/anthropic.claude-opus-4-6-v1",
  ]);
  ...
  const repeated = spawnSync(process.execPath, [cli, "--offline", "--list-models"], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      PI_CODING_AGENT_DIR: piAgentDir,
  ...
  assert.doesNotMatch(repeated.stdout + repeated.stderr, /No models match pattern/);
});
